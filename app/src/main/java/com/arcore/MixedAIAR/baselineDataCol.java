...
public class baselineDataCol implements Runnable {
    ...
    public baselineDataCol(MainActivity mInstance) {
...
//                        mInstance.acc_counter=0;
                   ...
                   /* nil commented since it changed re by mistake
                    ...
                    System.out.println(accmodel);
                    System.out.println("lastconsccounter");
                    System.out.println(mInstance.lastConscCounter);
                    if (accmodel && mInstance.lastConscCounter > 4) // the second condition is to skip change in nexttris for the first loop while we just had a change in tot tris
                    {
                        ...
                        if (tmpnextTris > 0) {
                            ...
                            try {
                                System.out.println("Running baseline alg");
...
//                curQ=0.3f;
    ...
    float baselineAlg(float tUP) throws InterruptedException {
        ...
        for (int i = 0 ; i < mInstance.objectCount; i++) {
            ...
            float R_i = tri_cnt_i / tri_max_i;                  // object i new decimation ratio R_i = T_i / T_i^max
            System.out.println("Ratio before adjustment.");
            ...
            float R_diff;                                       // current ratio difference
